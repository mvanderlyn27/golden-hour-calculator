...
function validate_inputs(spa) {
    ...
    if (spa.elevation < -6500000)
        return 11;
    if ((spa.function == types_1.FUNC_CODES.SPA_ZA_INC) || (spa.function == types_1.FUNC_CODES.SPA_ALL)) {
...
function earth_values(term_sum, jme) {
    let sum = 0;
...
function calculate_eot_and_sun_rise_transit_set(spa) {
    ...
    sun_rts.delta_t = 0;
    sun_rts.jd--;
    for (let i = 0; i < types_1.JD.JD_COUNT; i++) {
        calculate_geocentric_sun_right_ascension_and_declination(sun_rts);
        alpha[i] = sun_rts.alpha;
        delta[i] = sun_rts.delta;
        sun_rts.jd++;
    }
    m_rts[types_1.SUN.SUN_TRANSIT] = approx_sun_transit_time(alpha[types_1.JD.JD_ZERO], spa.longitude, nu);
    h0 = sun_hour_angle_at_rise_set(spa.latitude, delta[types_1.JD.JD_ZERO], h0_prime);
    if (h0 >= 0) {
        approx_sun_rise_and_set(m_rts, h0);
        for (let j = 0; j < types_1.SUN.SUN_COUNT; j++) {
        ...
        spa.srha = h_prime[types_1.SUN.SUN_RISE];
        spa.ssha = h_prime[types_1.SUN.SUN_SET];
        spa.sta = h_rts[types_1.SUN.SUN_TRANSIT];
        spa.suntransit = dayfrac_to_local_hr(m_rts[types_1.SUN.SUN_TRANSIT] - h_prime[types_1.SUN.SUN_TRANSIT] / 360.0, spa.timezone);
        spa.sunrise = dayfrac_to_local_hr(sun_rise_and_set(m_rts, h_rts, delta_prime, spa.latitude, h_prime, h0_prime, types_1.SUN.SUN_RISE), spa.timezone);
        spa.sunset = dayfrac_to_local_hr(sun_rise_and_set(m_rts, h_rts, delta_prime, spa.latitude, h_prime, h0_prime, types_1.SUN.SUN_SET), spa.timezone);
    }
    else {
        spa.srha = -99999;
        spa.ssha = -99999;
...
function spa_calculate(spa) {
    ...
    if (result === 0) {
        ...
        if (spa.function === types_1.FUNC_CODES.SPA_ZA_RTS || spa.function === types_1.FUNC_CODES.SPA_ALL) {
...
function test() {
    let input_spa = {
        ...
        elevation: 1830.14,
        function: types_1.FUNC_CODES.SPA_ALL,
        slope: 30,
        azm_rotation: -10,
    };
...
//
/////////////////////////////////////////////
//# sourceMappingURL=solar_calc.js.map