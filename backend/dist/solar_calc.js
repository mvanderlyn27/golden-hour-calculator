...
exports.calc_sun_position = void 0;
...
function get_years_since_1949(year) {
...
function get_mean_long_degrees(time) {
    return ((280.460 + 0.9856474 * time) % 360);
}
function get_mean_anomaly_rads(time) {
    return toRadians((357.528 + 0.9856003 * time) % 360);
}
function get_ecliptic_long_rads(mn_long, mn_anom) {
    return (toRadians((mn_long + 1.915 * Math.sin(mn_anom) + 0.020 * Math.sin(2 * mn_anom)) % 360));
}
function get_ecliptic_obliquity_rads(time) {
...
function greenwich_mean_sidereal_time_hours(time, hour) {
    return ((6.697375 + 0.0657098242 * time + hour) % 24);
}
function local_mean_sidereal_time_radians(gmst, lng) {
...
function calc_sun_position(time, lat, lng) {
...
exports.calc_sun_position = calc_sun_position;
let date = new Date("2021-11-14 12:00:00");
console.log(date);
console.log(date.getHours());
let data = calc_sun_position(date, 40, -105);
console.log("data:", data);
let times = suncalc.getTimes(date, 40, 105);
console.log(times);
console.log(times.goldenHour);
let pos = suncalc.getPosition(date, 40, 105);
