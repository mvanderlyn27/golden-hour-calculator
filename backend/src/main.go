...
func getGoldenHourTimeV1(c *gin.Context) {
	...
	times := suncalc.GetTimes(date, Input.Lat, Input.Long)
	out := SolarCalcOutput{
		StartTimeMorning: times[suncalc.Sunrise].Time.Unix(),
		EndTimeMorning:   times[suncalc.GoldenHourEnd].Time.Unix(),
		StartTimeNight:   times[suncalc.GoldenHour].Time.Unix(),
		EndTimeNight:     times[suncalc.Sunset].Time.Unix(),
	}
